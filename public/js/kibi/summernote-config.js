// Variables globales para Summernote KIBI
let createEditor = null;
let editEditor = null;

/*
 * CONFIGURACIÓN DE SUMMERNOTE PARA ARTÍCULOS KIBI
 * 
...
const summernoteConfig = {
    height: 300,
    lang: 'es-ES',
    placeholder: 'Escribe el contenido de tu artículo KIBI aquí...',
    toolbar: [
        ['style', ['style']],
    ...
    callbacks: {
        ...
        onInit: function() {
            console.log('Summernote KIBI initialized');
...
function deleteImageFromServer(imageUrl) {
    console.log('deleteImageFromServer KIBI called with URL:', imageUrl);
    
    const deleteUrl = '/kibi/articulos/delete-image';
    
    console.log('Delete URL KIBI:', deleteUrl);
    ...
    if (editForm && editForm.action) {
        const actionMatch = editForm.action.match(/\/([^\/]+)$/);
        if (actionMatch) {
...
function uploadImageToServer(file, editor) {
    console.log('uploadImageToServer KIBI called with file:', file);
    
    const formData = new FormData();
    formData.append('image', file);
    
    const uploadUrl = '/kibi/articulos/upload-image';
    console.log('Upload URL KIBI:', uploadUrl);
    ...
    if (editForm && editForm.action) {
        ...
        if (actionMatch) {
            articuloId = actionMatch[1];
            formData.append('articulo_id', articuloId);
            console.log('Article ID found for upload:', articuloId);
        }
    }
    ...
    if (!articuloId) {
        ...
        if (categoriaCheckboxes && categoriaCheckboxes.length > 0 && tituloInput) {
            ...
            console.warn('Could not find selected categorias or titulo input, using defaults');
    ...
    .then(data => {
        ...
        if (data.success && data.url) {
            ...
            console.log('Image inserted successfully:', data.url);
        } else {
            console.error('Error al subir imagen:', data);
            showNotification('Error al subir la imagen: ' + (data.error || 'Error desconocido'), 'error');
        }
    })
    .catch(error => {
...
function initializeCreateEditor() {
    console.log('Iniciando editor de crear KIBI...');
    ...
    setTimeout(() => {
        try {
            ...
            console.log('Inicializando Summernote en contenedor KIBI:', container);
            ...
            console.log('Editor de crear KIBI inicializado correctamente');
        } catch (error) {
            console.error('Error al inicializar editor de crear KIBI:', error);
...
function initializeEditEditor(content = '') {
    console.log('Iniciando editor de editar KIBI...');
    ...
    setTimeout(() => {
        try {
            ...
            console.log('Inicializando Summernote en contenedor de editar KIBI:', container);
            ...
            console.log('Editor de editar KIBI inicializado correctamente');
        } catch (error) {
            console.error('Error al inicializar editor de editar KIBI:', error);
            // Fallback: crear un textarea simple
...
function checkDependencies() {
    console.log('Verificando dependencias KIBI...');
    ...
    console.log('Todas las dependencias KIBI están disponibles');
    return true;
}
...
function debugEditorState(containerId) {
    ...
    console.log(`=== DEBUG EDITOR KIBI ${containerId} ===`);
...
function initSummernoteSimple(container, editorVar) {
    console.log('Inicializando Summernote KIBI de forma simple...');
    
    try {
        const editor = $(container).summernote(summernoteConfig);
        
        console.log('Summernote KIBI inicializado exitosamente');
        return editor;
    } catch (error) {
        console.error('Error al inicializar Summernote KIBI:', error);
        return null;
    }
}
...
function createFallbackEditor(container, type, content = '') {
    console.log(`Creando editor de fallback KIBI para ${type}`);
    ...
    textarea.placeholder = 'Escribe el contenido de tu artículo KIBI aquí...';
    ...
    console.log(`Editor de fallback KIBI creado para ${type}`);
}

// Función para inicializar el sistema cuando esté listo
function initializeSummernoteSystem() {
    console.log('Inicializando sistema de Summernote KIBI...');
    
    if (checkDependencies()) {
        console.log('Sistema de editores Summernote KIBI listo');
        return true;
    } else {
        console.error('Faltan dependencias para los editores Summernote KIBI');
        return false;
    }
}
...
function getCreateEditorContent() {
    if (createEditor && typeof $ !== 'undefined') {
        try {
            ...
            console.error('Error al obtener contenido del editor Summernote KIBI:', error);
...
function destroyCreateEditor() {
    if (createEditor && typeof $ !== 'undefined') {
        try {
            ...
            console.log('Error al destruir editor de crear KIBI:', e);
        }
        createEditor = null;
    }
...
function destroyEditEditor() {
    if (editEditor && typeof $ !== 'undefined') {
        try {
            ...
            console.log('Error al destruir editor de editar KIBI:', e);
        }
        editEditor = null;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM cargado, verificando dependencias de Summernote KIBI...');
    
    // Esperar más tiempo para asegurar que todas las dependencias estén cargadas
    setTimeout(() => {
        if (!initializeSummernoteSystem()) {
            console.error('Faltan dependencias para los editores Summernote KIBI, reintentando...');
            // Reintentar después de un tiempo
            setTimeout(() => {
                if (!initializeSummernoteSystem()) {
                    console.error('Las dependencias de Summernote KIBI siguen faltando después del reintento');
                }
            }, 1000);
        }
    }, 1000);
...
window.addEventListener('load', function() {
    console.log('Ventana completamente cargada, verificando sistema de Summernote KIBI...');
    setTimeout(() => {
        if (!initializeSummernoteSystem()) {
            console.error('Sistema de Summernote KIBI no inicializado después de carga completa');
        }
    }, 500);
});
...
window.closeSummernoteModals = function() {
    ...
    $('.note-modal-backdrop').remove();
    $('.note-modal').remove();
    console.log('Modales de Summernote KIBI cerrados');
};

// Función para prevenir que los modales se bloqueen
window.preventSummernoteModalBlock = function() {
    console.log('Setting up Summernote modal prevention for KIBI...');
    ...
    $(document).on('click', '.note-btn-image', function(e) {
        ...
        console.log('Image button clicked in Summernote KIBI');
    ...
    // También interceptar el drag and drop
    $(document).on('drop', '.note-editable', function(e) {
        e.preventDefault();
        const files = e.originalEvent.dataTransfer.files;
        if (files.length > 0 && files[0].type.startsWith('image/')) {
            console.log('File dropped in Summernote KIBI:', files[0].name);
            uploadImageToServer(files[0], $(this).summernote('editor'));
        }
    });
...
console.log('✅ SummernoteManager KIBI cargado correctamente');
console.log('SummernoteManager KIBI disponible:', !!window.SummernoteManager);
