// Funciones para el catálogo de casos de éxito asíncrono
document.addEventListener('DOMContentLoaded', function() {
    // Obtener elementos del DOM
    const casosExitoGrid = document.querySelector('.casos-exito-grid');
    const paginationContainer = document.querySelector('.pagination-container');
    
    // Agregar funcionalidad de clic en toda la tarjeta
    function attachCardClickListeners() {
        const casoCards = document.querySelectorAll('.caso-exito-card');
        casoCards.forEach(card => {
            card.addEventListener('click', function(e) {
                // Evitar el clic si se hace clic en un enlace o botón específico
                if (e.target.closest('a') || e.target.closest('button') || e.target.closest('.group/more')) {
                    return;
                }
                
                const url = this.getAttribute('data-caso-url');
                if (url) {
    ...
    function showLoading() {
        // Mostrar loading siempre, incluso si no hay casos de éxito
        const loadingIndicator = document.getElementById('loading-indicator');
        if (loadingIndicator) {
            loadingIndicator.style.display = 'flex';
        }
        
        // Aplicar efecto de carga al grid, incluso si está vacío
        const grid = document.querySelector('.casos-exito-grid');
        if (grid) {
    ...
    function hideLoading() {
        ...
        // Restaurar efecto del grid, incluso si está vacío
        const grid = document.querySelector('.casos-exito-grid');
        if (grid) {
    ...
    function performSearch(categoria = null, busqueda = null) {
        ...
        .then(html => {
            ...
            const newGrid = tempDiv.querySelector('.casos-exito-grid');
            const currentGrid = document.querySelector('.casos-exito-grid');
            if (newGrid && currentGrid) currentGrid.innerHTML = newGrid.innerHTML;
            
            // Reagregar listeners de clic a las nuevas tarjetas
            attachCardClickListeners();
