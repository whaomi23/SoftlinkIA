...
/*
 * CONFIGURACIÓN DE SUMMERNOTE PARA CASOS DE ÉXITO
 * 
...
const summernoteConfig = {
    height: 300,
    lang: 'es-ES',
    placeholder: 'Escribe el contenido de tu caso de éxito aquí...',
    toolbar: [
        ['style', ['style']],
...
function deleteImageFromServer(imageUrl) {
    ...
    const deleteUrl = isAdmin ? '/admin/casos-exito/delete-image' : '/creador/casos-exito/delete-image';
    
    console.log('Delete URL:', deleteUrl);
    console.log('Is Admin:', isAdmin);
    
    // Intentar obtener el ID del caso de éxito si estamos editando
    let casoExitoId = null;
    
    // Para el modal de editar, obtener el ID del caso de éxito
    const editForm = document.getElementById('editForm');
    if (editForm && editForm.action) {
        const actionMatch = editForm.action.match(/\/(\d+)$/);
        if (actionMatch) {
            casoExitoId = actionMatch[1];
            console.log('Caso de éxito ID found for deletion:', casoExitoId);
        }
    }
    ...
    if (casoExitoId) {
        formData.append('caso_exito_id', casoExitoId);
    }
...
function uploadImageToServer(file, editor) {
    ...
    // Determinar la URL correcta según el contexto
    const isAdmin = window.location.pathname.includes('/admin/');
    const uploadUrl = isAdmin ? '/admin/casos-exito/upload-image' : '/creador/casos-exito/upload-image';
    ...
    // Para casos de éxito nuevos o cuando se necesita obtener categoría y título del formulario
    if (!casoExitoId) {
        ...
        if (categoriaCheckboxes && categoriaCheckboxes.length > 0 && tituloInput) {
            ...
            const titulo = tituloInput.value || 'caso-exito';
            ...
            formData.append('titulo', 'caso-exito');
        }
    }
...
function createFallbackEditor(container, type, content = '') {
    ...
    textarea.placeholder = 'Escribe el contenido de tu caso de éxito aquí...';
...
// Función de utilidad para mostrar notificaciones con SweetAlert2
