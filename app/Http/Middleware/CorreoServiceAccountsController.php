...
class CorreoServiceAccountsController extends Controller
{
     ...
     */
    public function enviarEmailVerificacionNuevoUsuario(User $user)
    {
        try {
            ...
            // Enviar email
            $this->enviarEmailVerificacion($user, $token, 'nuevo_usuario');
     ...
     */
    public function enviarEmailVerificacionUsuarioExistente(User $user)
    {
        try {
            // Generar token de verificación
            $token = $user->generateEmailVerificationToken();
            ...
            // Enviar email
            $this->enviarEmailVerificacion($user, $token, 'usuario_existente');
     ...
     */
    private function enviarEmailVerificacion(User $user, string $token, string $tipo = 'verificacion', string $sessionToken = null)
    {
        try {
            $data = [
                'user' => $user,
