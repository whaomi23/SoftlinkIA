...
<span class="inline-flex items-center {{ $classes['container'] }} text-black bg-gradient-to-r from-{{ $color }}-100 to-{{ $color }}-200 rounded-full border border-{{ $color }}-300 group-hover:from-{{ $color }}-200 group-hover:to-{{ $color }}-300 group-hover:border-{{ $color }}-400 transition-all duration-500">
    <span class="material-icons {{ $classes['icon'] }} text-{{ $color }}-600">{{ $icon }}</span>
