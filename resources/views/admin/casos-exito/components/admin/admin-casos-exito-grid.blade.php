...
<!-- Success Cases Grid -->
<div id="casosExitoGrid" class="relative z-20 grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-full">
    @forelse($casosExito as $casoExito)
        <div
            ...
            <div class="absolute inset-0 bg-gradient-to-br from-indigo-600/10 via-blue-500/5 to-cyan-600/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
            ...
            @php
                $cardImg = $casoExito->url_imagen
                    ? Storage::url($casoExito->url_imagen)
            ...
            @endphp
                <div class="relative h-48 bg-gradient-to-br from-slate-800/50 via-slate-700/30 to-slate-900/50 flex items-center justify-center overflow-hidden flex-shrink-0">
                    ...
                    @else
                        <div class="w-full h-full bg-gradient-to-br from-slate-700/80 to-slate-800/80 flex items-center justify-center group-hover:from-slate-600/80 group-hover:to-slate-700/80 transition-all duration-500">
                            <span class="material-icons text-cyan-300 text-6xl opacity-80 group-hover:opacity-100 group-hover:rotate-12 transition-all duration-500">emoji_events</span>
                        </div>
                    @endif
                ...
                <div class="absolute top-4 right-4">
                    <span class="inline-flex items-center px-3 py-1.5 text-xs font-semibold rounded-full backdrop-blur-sm
                            @if($casoExito->status === 'publicado') bg-green-500/90 text-white
                            @elseif($casoExito->status === 'borrador') bg-yellow-500/90 text-white
                            @else bg-gray-500/90 text-white
                            @endif">
                        <span class="material-icons text-xs mr-1">
                            @if($casoExito->status === 'publicado') visibility
                            @elseif($casoExito->status === 'borrador') edit
                            @else archive
                            @endif
                        </span>
                        {{ ucfirst($casoExito->status) }}
                    </span>
                </div>
            </div>
                ...
                <div class="p-6 flex flex-col flex-grow overflow-hidden">
                    ...
                    <div class="flex flex-col gap-3 mb-4 flex-shrink-0">
                        <div class="flex items-center flex-wrap gap-1 min-h-[2rem] break-words">
                            ...
                            @foreach(array_slice($categorias, 0, 3) as $categoria)
                                @include('admin.casos-exito.components.shared.category-casos-exito-tag', ['categoria' => $categoria, 'size' => 'small'])
                            @endforeach
                    ...
                    <div class="flex items-center justify-between text-sm text-slate-400 group-hover:text-slate-300 mb-6 transition-colors duration-500 flex-shrink-0">
                        <span class="flex items-center">
                            <span class="material-icons text-sm mr-2 text-cyan-500">person</span>
                            {{ $casoExito->autor->name }} {{ $casoExito->autor->apellido_paterno }}
                            {{ $casoExito->autor->apellido_materno }}
                        </span>
                    </div>

                    <!-- Nivel de Dificultad y Fecha -->
                    <div class="flex items-center justify-between mb-6">
                        @include('admin.casos-exito.components.shared.difficulty-casos-exito-tag', ['nivel' => $casoExito->nivel_dificultad, 'size' => 'small'])
                        <span class="text-sm text-slate-400 group-hover:text-slate-300 transition-colors duration-500 flex items-center">
                            <span class="material-icons text-sm mr-1">schedule</span>
                            {{ $casoExito->created_at->format('d/m/Y') }}
                        </span>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-slate-700/50 flex-shrink-0">
                        ...
                        <div class="flex space-x-3">
                            <a href="{{ route('admin.casos-exito.show', $casoExito) }}"
                            ...
                            <button onclick="openEditModal('{{ $casoExito->slug }}')"
                                class="group/link relative overflow-hidden bg-gradient-to-br from-indigo-500 to-cyan-500 text-white w-12 h-12 rounded-2xl transition-all duration-500 transform hover:scale-110 hover:shadow-lg hover:shadow-cyan-500/30 flex items-center justify-center"
                        ...
                        <button onclick="openDeleteModal('{{ $casoExito->slug }}', '{{ $casoExito->titulo }}')"
            ...
            <!-- Borde de neón -->
            <div class="absolute inset-0 rounded-2xl border-2 border-cyan-400/30 group-hover:border-cyan-300 group-hover:shadow-[0_0_20px_rgba(34,211,238,0.4)] transition-all duration-500"></div>
        </div>
    @empty
        <div class="col-span-full">
            <div class="text-center py-20">
                ...
                <p class="text-gray-600 dark:text-gray-400 mb-8 max-w-md mx-auto">Comienza creando tu primer
                    caso de éxito y comparte tu conocimiento con el mundo</p>
                <button onclick="openCreateModal()"
                    class="group relative inline-flex items-center px-8 py-4 bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-600 text-white font-semibold rounded-2xl shadow-lg shadow-cyan-500/25 hover:shadow-xl hover:shadow-cyan-500/40 transition-all duration-300 hover:scale-105 hover:-translate-y-1">
                    <div
                        class="absolute inset-0 bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-600 rounded-2xl blur opacity-75 group-hover:opacity-100 transition-opacity">
                    </div>
                    <span class="material-icons mr-3 relative z-10">add_circle</span>
                    <span class="relative z-10">Crear mi primer caso de éxito</span>
                </button>
            </div>
        </div>
    @endforelse
