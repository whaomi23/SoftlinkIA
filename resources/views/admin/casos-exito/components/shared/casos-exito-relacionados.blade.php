@props(['casosExito', 'routeName' => 'casos-exito.ver', 'titulo' => null])
...
<div class="py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="relative">
            <div class="overflow-x-auto scrollbar-hide" id="related-casos-exito-scroll" style="scrollbar-width: none; -ms-overflow-style: none;">
                <div class="flex space-x-6 pb-4" style="width: max-content;">
                    @foreach($casosExito as $casoExitoRelacionado)
                        <article class="group glass-effect-enhanced rounded-lg shadow-lg border border-slate-700/50 overflow-hidden hover:shadow-xl transition-all duration-500 transform hover:scale-105 flex-shrink-0 bg-slate-800/30 backdrop-blur-sm relative" style="width: 320px;">
                            ...
                            <a href="{{ route($routeName, $casoExitoRelacionado) }}" class="block relative z-10">
                                <!-- Imagen del caso de éxito (16:9 y completa sin recortar) -->
                                <div class="relative w-full overflow-hidden bg-slate-700/50">
                                    <div class="pt-[56.25%]"></div>
                                    @php
                                        // Usar la misma lógica que el catálogo
                                        $cardImg = $casoExitoRelacionado->url_imagen
                                            ? Storage::url($casoExitoRelacionado->url_imagen)
                                            : $casoExitoRelacionado->url_imagen_banner;
                                    @endphp
                                    
                                    @if($cardImg)
                                        <img src="{{ $cardImg }}" alt="{{ $casoExitoRelacionado->titulo }}"
                                    ...
                                    @else
                                        <div class="absolute inset-0 w-full h-full bg-gradient-to-br from-slate-700/80 to-slate-800/80 flex items-center justify-center">
                                            <span class="material-icons text-cyan-300 text-4xl opacity-80">emoji_events</span>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="p-6">
                                    ...
                                    @if($casoExitoRelacionado->categoria)
                                        <div class="flex flex-wrap gap-2 mb-3 group">
                                            @php
                                                $categorias = is_string($casoExitoRelacionado->categoria) 
                                                    ? array_unique(array_filter(array_map('trim', explode(',', $casoExitoRelacionado->categoria))))
                                                    : [$casoExitoRelacionado->categoria];
                                            @endphp
                                            @if(count($categorias) > 0)
                                                @include('admin.casos-exito.components.shared.category-casos-exito-tag', ['categoria' => $categorias[0], 'size' => 'small'])
                                    ...
                                    <h3 class="text-lg font-semibold text-white mb-2 line-clamp-2">
                                        {{ $casoExitoRelacionado->titulo }}
                                    </h3>
                                    
                                    @if($casoExitoRelacionado->subtitulo)
                                        <p class="text-slate-300 text-sm mb-4 line-clamp-2">
                                            {{ Str::limit($casoExitoRelacionado->subtitulo, 100) }}
                                        </p>
                                    @endif
                                    
                                    <!-- Nivel de dificultad -->
                                    @if($casoExitoRelacionado->nivel_dificultad)
                                        <div class="mb-3">
                                            @include('admin.casos-exito.components.shared.difficulty-casos-exito-tag', ['nivel' => $casoExitoRelacionado->nivel_dificultad, 'size' => 'small'])
                                        </div>
                                    @endif
                                    ...
                                    <div class="flex items-center justify-between text-xs text-slate-400">
                                        <div class="flex items-center">
                                            ...
                                            <span>{{ $casoExitoRelacionado->created_at->format('d/m/Y') }}</span>
                                        </div>
                                        <div class="flex items-center">
                                            <span class="material-icons text-xs mr-1">visibility</span>
                                            <span>{{ ucfirst($casoExitoRelacionado->status) }}</span>
                                        </div>
                                    </div>
                                </div>
                            </a>
...
document.addEventListener('DOMContentLoaded', function() {
    // Scroll automático para casos de éxito relacionados
    function initRelatedCasosExitoScroll() {
        const scrollContainer = document.getElementById('related-casos-exito-scroll');
        if (!scrollContainer) return;

        let isScrolling = false;
        const scrollSpeed = 1; // píxeles por frame
        const casoExitoWidth = 350; // ancho de cada caso de éxito + espacio
