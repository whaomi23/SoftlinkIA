...
<div class="glass-effect-enhanced rounded-lg shadow-lg border border-slate-700/50 p-3 mb-6 bg-slate-800/30 backdrop-blur-sm">
    <div class="px-3 py-2 flex items-center justify-between cursor-pointer toc-header">
        <h4 class="text-white font-medium text-sm">Tabla de contenido</h4>
        <svg class="w-4 h-4 text-purple-400 transition-transform duration-300 toc-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <nav class="space-y-1">
            ...
            @if(!empty($finalSections) && count($finalSections) > 0)
                @foreach($finalSections as $s)
                    ...
                    @endphp
                    <a href="#{{ $secId }}" 
                       class="block text-sm text-slate-300 hover:text-cyan-400 transition-colors py-2 toc-link"
                       data-target="{{ $secId }}">
                        {{ $title }}
                    </a>
                @endforeach
            @else
                <p class="text-sm text-slate-400 py-2">No hay secciones disponibles</p>
            @endif
        </nav>
    </div>
</div>
